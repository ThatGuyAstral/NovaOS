...
// thanks, iiPilix for the driver!
// modified a little bit

namespace NovaOS.Core
{
    public class AHCI_SATA
    {
        public static Cosmos.Debug.Kernel.Debugger ahciDebugger = new Cosmos.Debug.Kernel.Debugger("AHCI");
        ...
        /// </summary>
        public static void Init(bool silent)
        {
            try
            {
                ...
                foreach (var dvc in PCI.Devices)
                {
                    ...
                    if (!silent)
                        Console.WriteLine($"PCI Device {i} | VendorID: {dvc.VendorID} |  BAR0: {dvc.BAR0}  | ProgIF: {dvc.ProgIF}  |  BUS: {dvc.bus} | Function: {dvc.function} | Slot: {dvc.slot} |  Class: {dvc.ClassCode}  | Subclass: {dvc.Subclass} | CMD: {GetCMD(dvc.Command)}");
                    ...
                    if (dvc.ClassCode == 1 && dvc.Subclass == 6)
                    {
                        ...
                        StoragePort.Devices.ForEach(device => { Console.WriteLine($"Found storage device. Type: {GetBlockType(device.Type)} | BlockSize: {device.BlockSize} | BlockCount: {device.BlockCount}"); });
                        if (!silent)
                            Console.WriteLine($"ADDR: {aAddressX} | PortNum: {mPortNum}");
                        PortRegisters prtg = new PortRegisters(aAddressX, mPortNum);
                        Console.WriteLine($"Is PTRG active: {prtg.Active} | PortNum: {prtg.mPortNumber} | PortCMD: {prtg.CMD} | Reserved: {prtg.Reserved}");
                        ...
                        if (!silent)
                            Console.WriteLine($"\nAmount of SATA Devices: {satadevices.Count}\n");
                        int idx = 0;
                        foreach (var satadevice in satadevices)
                        {
                            if (idx < satadevices.Count)
                                idx++;
                            if (!silent)
                                Console.WriteLine($"SATA Device {idx} | Type: {GetBlockType(satadevice.Type)} | BlockSize = {satadevice.BlockSize} | BlockCount = {satadevice.BlockCount}");
                        }
                    }
                }
            }
            catch (Exception ex) { Kernel.Crash(ex); }
        }

        /// <summary>
        /// Check if SATA was successfuly initialized
        /// </summary>
        public static void Check()
        {
            List<BlockDevice> satadevices = SATA.Devices;
            Console.WriteLine($"Amount of devices found: {satadevices.Count}");
        }
        ...
        private static string GetPType(PortType port)
        {
            switch (port)
            {
        ...
        private static string GetBlockType(BlockDeviceType type)
        {
            switch (type)
            {
        ...
        private static void PortRebase(PortRegisters aPort, uint aPortNumber)
        {
        ...
        private static PortType CheckPortType(PortRegisters aPort)
        {
        ...
        private static bool StartCMD(PortRegisters aPort)
        {
            int i;
            for (i = 0; i < 101; i++)
            {
        ...
        private static bool StopCMD(PortRegisters aPort)
        {
